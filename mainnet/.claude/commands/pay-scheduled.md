Trigger a scheduled payment from a BittyVault on Ethereum mainnet. Permissionless — anyone may call it once the payment is due (unless the payment has a trigger address, in which case only that address can).
