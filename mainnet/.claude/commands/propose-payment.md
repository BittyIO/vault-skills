Propose a new scheduled payment on a BittyVault on Ethereum mainnet, as a **payout operator**. The proposal sits pending until the vault owner approves it (in the Bitty web app); only then can it pay.
