Propose a one-off send from a BittyVault on Sepolia, as a **payout operator**. The batch is validated against the operator's rolling quota immediately, then queued until the vault owner approves it (in the Bitty web app); nothing moves before approval.
